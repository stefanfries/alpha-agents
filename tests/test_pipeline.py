...
@pytest.mark.asyncio
async def test_run_monitoring_keeps_roll_when_replacement_found(monkeypatch):
    ...
    run = {
        "stages": {
            "screening": {"result": screening.model_dump(mode="json")},
            "research": {"result": {"fundamentals": {"A": {"currentPrice": 100.0}}}},
        },
        "config_overrides": {"portfolio": {"max_positions": 5}},
    }
    ...
    assert len(result.positions_to_roll) == 1
    assert result.positions_to_roll[0].roll_replacement is not None
    assert result.positions_to_roll[0].roll_replacement.warrant_isin == "ISIN2"
    assert len(result.positions_to_sell) == 0


@pytest.mark.asyncio
async def test_run_monitoring_holds_when_roll_replacement_is_worse(monkeypatch):
    from app.models.signals import RollReplacement
    from app.orchestrator import Pipeline
...
@pytest.mark.asyncio
async def test_find_roll_replacement_queries_warrant_selection_agent(monkeypatch):
    import app.orchestrator as orchestrator_module
    from app.models.signals import SelectedWarrant, WarrantSelectionResult
    from app.orchestrator import Pipeline
    ...
    class FakeWarrantSelectionAgent:
        def __init__(self, **kwargs):
            self._kwargs = kwargs

        async def run(self, input: SelectionResult) -> WarrantSelectionResult:
            assert len(input.selected) == 1
            assert input.selected[0].symbol == "A"
            return WarrantSelectionResult(
                selected=[
                    SelectedWarrant(
                        underlying=input.selected[0],
                        warrant_isin="ROLLISIN1",
                        warrant_wkn="ROLLWKN1",
                        strike=123.0,
                        maturity_date=date.today() + timedelta(days=180),
                        spread_pct=1.1,
                        leverage=4.0,
    ...
    monkeypatch.setattr(orchestrator_module, "FinHubTool", FakeFinHubTool)
    monkeypatch.setattr(orchestrator_module, "WarrantSelectionAgent", FakeWarrantSelectionAgent)
    monkeypatch.setattr(orchestrator_module.warrant_availability, "overrides_map", fake_overrides_map)

    pipeline = Pipeline()
    screening = SelectionResult(
        selected=[Ticker(symbol="A", isin="UNDERISIN1")],
        scores={"A": 1.0},
        rationale={},
    )
    run = {
        ...
        "config_overrides": {"warrant_selection": {}},
    }

    replacement = await pipeline._find_roll_replacement(
        run=run,
        screening=screening,
        underlying_symbol="A",
    )
    ...
    assert replacement.warrant_isin == "ROLLISIN1"
    assert replacement.warrant_wkn == "ROLLWKN1"
